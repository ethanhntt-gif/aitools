        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_360px]">
          <Surface className="overflow-hidden">
            ...
            <div className="space-y-6 p-6 sm:p-8">
              {isPreview ? <div className="inline-flex items-center rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700 ring-1 ring-amber-200 dark:bg-amber-500/10 dark:text-amber-300 dark:ring-amber-500/30">Private preview</div> : null}
              <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
                {activeProject.logo_url ? <img className="h-20 w-20 rounded-3xl border border-slate-200 object-cover dark:border-slate-700" src={activeProject.logo_url} alt={activeProject.title} /> : null}
                 <div className="space-y-3">
                   <div className="flex flex-wrap gap-2">
                    ...
                    {getProjectCategories(activeProject).map((categoryItem) => <button className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600 transition hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700" key={categoryItem} onClick={() => openCategoryPage(categoryItem)} type="button">{categoryItem}</button>)}
                   </div>
                  <h3 className="text-3xl font-semibold tracking-tight text-slate-950 dark:text-slate-100">{activeProject.title}</h3>
                  {activeProject.slogan ? <p className="text-lg leading-8 text-slate-600 dark:text-slate-300">{activeProject.slogan}</p> : null}
                </div>
              </div>
              <p className="text-base leading-8 text-slate-600 dark:text-slate-300">{activeProject.description || "No description provided yet."}</p>
            </div>
          </Surface>
          <div className="space-y-6">
            <Surface className="p-6"><p className="text-xs font-semibold uppercase tracking-[0.28em] text-sky-600 dark:text-sky-400">Author</p><button className="mt-4 flex w-full items-center gap-4 rounded-[22px] border border-slate-200 bg-white px-4 py-4 text-left transition hover:border-slate-300 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-950 dark:hover:border-slate-600 dark:hover:bg-slate-900" onClick={() => openAuthorProfile(activeProject.owner_id)} type="button"><div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-[22px] border border-slate-200 bg-[linear-gradient(135deg,_#0f172a_0%,_#0369a1_100%)] text-xl font-semibold text-white shadow-sm dark:border-slate-700">{activeAuthorProfile?.avatar_url ? <img alt={activeAuthorProfile.display_name} className="h-full w-full object-cover" src={activeAuthorProfile.avatar_url} /> : getAuthorInitial(activeProject.owner_email)}</div><div className="min-w-0"><p className="text-lg font-semibold text-slate-950 dark:text-slate-100">{activeAuthorProfile?.display_name || formatAuthorName(activeProject.owner_email)}</p><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{activeAuthorProfile?.headline || "Open author profile"}</p></div></button></Surface>
            <Surface className="p-6"><p className="text-xs font-semibold uppercase tracking-[0.28em] text-sky-600 dark:text-sky-400">Visit project</p><a className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-sky-600 dark:bg-sky-500 dark:hover:bg-sky-400" href={activeProject.project_url || "#"} target="_blank" rel="noreferrer">Visit website<ArrowUpRightIcon className="h-4 w-4" /></a></Surface>
            <Surface className="overflow-hidden">
